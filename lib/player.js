class Player extends Entity {
  ...
  draw() {
    ...
    //   this.manager.addEvent("shake", 10);
    // }
    // super.draw();
  ...
  static createPlayer(manager) {
    ...
    car.addState("default", (c) => {
      ...
      if (c.collision.isColliding) {
        c.manager.addEvent(Player.Events.FALL, 10);
      }
      c.entityDraw();
      ...
      if (c.manager.hasEvent(Player.Events.FALL)) c.setState("spin");
    });
    ...
    manager.addBehavior("shaking", (m) => {
      if (m.hasEvent(Player.Events.FALL)) {
